...
export function getFormatedAmount(price: any) {
  if (price) {
    ...
    price = parseFloat(String(price)).toFixed(
      String(price).includes("-") ? parseInt(String(price).split("-")[1]) + 2 : String(price).split(".")[1]?.length || 0
    );
...
export function formatBigAmount(amount: number | string, precision = 3) {
  ...
  if (precision) {
    return `${amount.split(",")[0]}.${amount
      ...
      .slice(1)
      .join("")
      .slice(0, precision - amount.split(",")[0].length)}${letter}`;
  }
...
export function getTokenFormattedPrice(
  ...
  if (price) {
    ...
    if (parseFloat(price) < 0.0001) {
      price = parseFloat(String(price)).toFixed(
        String(price).includes("-") ? parseInt(String(price).split("-")[1]) + 2 : String(price).split(".")[1].length
      );

      const exp = price.match(/0\.0+[1-9]/)?.[0] || "";
      return (
        <Flex mt={marginTop || "-45px"} justify={justify || "center"} align="center">
...
export function getShortenedAmount(number: number, precision?: number): string {
  ...
  if (number < 0.0001) {
    const bufferNumber = parseFloat(String(number)).toFixed(
      String(number).includes("-")
        ? parseInt(String(number).split("-")[1]) + 2
