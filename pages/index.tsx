...
export async function getStaticProps() {
  ...
  const { data } = await supabase
    .from("assets")
    .select(
      "market_cap,volume,logo,volume,name,symbol,twitter,website,chat,discord,price_change_24h,price_change_7d,price,rank_change_24h,id,contracts,blockchains,pairs,liquidity"
    )
  ...
  const { data: gainers } = await supabase
    .from("assets")
    .select("name,price_change_24h,logo,id,liquidity,contracts")
    .filter("volume", "gt", 50000)
  ...
  const { data: recents } = await supabase
    .from("assets")
    .select("name,price_change_24h,logo,id")
