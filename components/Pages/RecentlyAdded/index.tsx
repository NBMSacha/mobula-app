...
export default function RecentlyAdded({ tokensBuffer }) {
  ...
  useEffect(() => {
    ...
    supabase
      .from("assets")
      .select(
        "id,name,price_change_24h,volume,symbol,logo,market_cap,price,rank,contracts,blockchains,twitter,website,chat,created_at"
      )
  ...
  return (
    <Flex justify="center" maxWidth="1850px" mx="auto">
      <div className={styles.dflex}>
        <Flex>
          <Text display={["flex", "flex", "none", "none"]} mb="20px" mt="25px">
            Recently added
          </Text>
        </Flex>
        <Flex
          ...
          className={styles.stickyFix}
          w="100%"
          align="end"
          justify="space-between"
        >
          <Flex direction="column">
            <Heading mb="15px" fontSize="24px" fontFamily="Inter">
              Recently added tokens
            </Heading>
            <Text whiteSpace="normal" fontSize={["12px", "12px", "14px", "14px"]}>
              Latest tokens who got validated by the{" "}
              <span style={{ color: "var(--chakra-colors-blue)" }}>Mobula DAO</span>
            </Text>
          </Flex>
          <Text display={["none", "none", "none", "flex"]}>
            You can submit your own token (or a token you support)
            <Link color="blue" ml="5px">
              here
            </Link>
          </Text>
        </Flex>
        <Widget
          ...
          setVisible={setWidgetVisibility}
        />
        <BlockchainBtn
        ...
        <TableContainer mb="20px" mt="20px">
          <Table variant="simple">
            <Thead
              borderTop="2px solid var(--box_border)"
            ...
            >
              <Tr>
                <Th
                  ...
                  position="sticky"
                  left="0px"
                  bg={["var(--background)", "var(--background)", "var(--background)", "none"]}
                  textAlign="start"
                >
                  Asset
                </Th>
            ...
            {tokens.map((token: any) => {
              ...
              return (
                <Tbody
                  fontSize={["12px", "12px", "16px", "16px"]}
                  py="5px"
                  onClick={() => router.push(`/asset/${getUrlFromName(token.name)}`)}
                ...
                >
                  <Tr>
                    <Td
                      ...
                      py="5px"
                      left="0px"
                      bg={["var(--background)", "var(--background)", "none"]}
                    >
                      <Flex align="center">
                        <Text pl="30px" opacity="1" mr="10px" display={["none", "none", "flex", "flex"]}>
                          {token.rank}
                        </Text>
                        <Image
                          borderRadius="50%"
                          h={["25px", "25px", "30px", "30px"]}
                          w={["25px", "25px", "30px", "30px"]}
                          src={token.logo}
                          mr="10px"
                        />
                    ...
                    <Td borderBottom="1px solid var(--box_border) !important" px="5px" py="5px" isNumeric>
                      <Text>
                        {getTokenFormattedPrice(token.price, "$", {
                          justify: "right",
                          marginTop: "auto",
                        })}
                      </Text>
                    ...
                    <Td borderBottom="1px solid var(--box_border) !important" px="5px" py="5px" isNumeric>
                      <Text color={getTokenPercentage(token.price_change_24h) > 0.01 ? "green" : "red"}>
                        {getTokenPercentage(token.price_change_24h) > 0.01 ? (
                          <TriangleUpIcon mr="5px" />
                        ) : (
                          <TriangleDownIcon mr="5px" />
                        )}
                        {getTokenPercentage(token.price_change_24h)}%
                      </Text>
                    </Td>
                    <Td borderBottom="1px solid var(--box_border) !important" px="5px" py="5px" isNumeric>
                      <Text>${formatAmount(token.market_cap)}</Text>
                    </Td>
                    <Td borderBottom="1px solid var(--box_border) !important" px="5px" py="5px" isNumeric>
                      <Text>${formatAmount(token.volume)}</Text>
                    </Td>
