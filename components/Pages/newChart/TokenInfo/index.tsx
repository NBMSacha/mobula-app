...
const TokenInfo = ({
  price24hLow, price24hHigh, baseAsset, setSelectorInfo, selectorInfo, totalScore,
}) => {
  ...
  return (
    ...
    >
      ...
      <Flex px="20px" fontFamily="Inter" w="100%" justify="space-between" mb={["", "", "", "30px"]}>
        ...
        <Flex align="center">
          <Image
            mr={["8px", "8px", "15px", "15px"]}
            src={baseAsset.logo}
            h={["32px", "32px", "38px", "48px"]}
          />
          <Box>
            <Flex align="center">
              <Text
        ...
        <Flex align="center" display={["none", "none", "none", "flex"]}>
          <Link
            target="_blank"
            href={baseAsset.website}
            borderRadius="6px"
            border="1px solid var(--box_border)"
            _focus={{ boxShadow: "none" }}
            color={selectorInfo === "Website" ? "white" : "none"}
            onClick={() => setSelectorInfo("Website")}
            mr="6px"
            bg={selectorInfo === "Website" ? "blue" : "var(--btnInfo)"}
            fontSize="12px"
          ...
          {baseAsset.audit
            ? (
              <Link
                ...
                border="1px solid var(--box_border)"
                _focus={{ boxShadow: "none" }}
                color={selectorInfo === "Audit" ? "white" : "none"}
                onClick={() => setSelectorInfo("Audit")}
                bg={selectorInfo === "Audit" ? "blue" : "var(--btnInfo)"}
                mr="6px"
                fontSize="12px"
                _hover={{ textDecoration: "none" }}
              >
          ...
          {baseAsset.kyc
            ? (
              <Link
                ...
                border="1px solid var(--box_border)"
                _focus={{ boxShadow: "none" }}
                color={selectorInfo === "KYC" ? "white" : "none"}
                onClick={() => setSelectorInfo("KYC")}
                bg={selectorInfo === "KYC" ? "blue" : "var(--btnInfo)"}
                mr="6px"
                fontSize="12px"
                _hover={{ textDecoration: "none" }}
              >
          ...
          {baseAsset.description !== "" && (
            <Link
              ...
              _focus={{ boxShadow: "none" }}
              onClick={() => {
                if (selectorInfo !== "Infos") {
                  setSelectorInfo("Infos");
                } else {
                  setSelectorInfo("");
                }
              }}
              color={selectorInfo === "Infos" ? "white" : "none"}
              bg={selectorInfo === "Infos" ? "blue" : "var(--btnInfo)"}
              mr="6px"
              fontSize="12px"
              _hover={{ textDecoration: "none" }}
            >
        ...
        <Flex direction="column" align="center" justify="center" ml="20px">
          ...
          <Flex align="center">
            <Text
              mr={["", "", "", "10px"]}
              fontSize={["16px", "16px", "30px", "30px"]}
            >
              {getTokenFormattedPrice(baseAsset.price, "$", {
                justify: "right",
                marginTop: "auto",
              })}
            </Text>
            <Text
              color={getTokenPercentage(baseAsset.price_change_24h) > 0 ? "green" : "red"}
              ml={["10px", "", "", ""]}
              fontSize="12px"
            >
              {getTokenPercentage(baseAsset.price_change_24h) > 0 ? `+${getTokenPercentage(baseAsset.price_change_24h)}` : getTokenPercentage(baseAsset.price_change_24h)}
              %
            </Text>
          </Flex>
          {/* Progress bar */}
          <Flex direction="column" w="100%">
            <Flex h={["3px", "3px", "5px", "5px"]} w="100%" bg="#87878720" borderRadius="8px">
              <Box
                bg={getTokenPercentage(baseAsset.price_change_24h) > 0 ? "green" : "red"}
                h="100%"
                w={`${((baseAsset.price - price24hLow) / (price24hHigh - price24hLow)) * 100}%`}
                borderRadius="8px"
              />
            </Flex>
            <Flex mt="3px" justify="space-between" fontWeight="300">
              <Text fontSize={["8px", "8px", "9px", "9px"]}>
                Low: $
                {getFormatedAmount(price24hLow)}
              </Text>
              <Text fontSize={["8px", "8px", "9px", "9px"]}>
                High $
                {getFormatedAmount(price24hHigh)}
              </Text>
            </Flex>
          </Flex>
