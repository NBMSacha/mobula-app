...
const MobileInfo = ({ baseAsset, moreStat, totalScore }) => {
  ...
  return (
    <Box w="90%" display={moreStat ? "block" : "none"}>
      <Text p="0px 25px" fontSize="10px" lineHeight="18px">
        {baseAsset.description}
      </Text>
      <Flex justify="space-between" w="100%" p="0px 25px" mt="20px">
        <Box mb="20px">
          <Text fontSize="9px" opacity=".6">
            Marketcap
          </Text>
          ...
          <Text fontSize="12px">${formatAmount(baseAsset.market_cap_diluted)}</Text>
        </Box>
        <Box mb="20px">
          <Text fontSize="9px" opacity=".6">
            Circulating supply
          ...
          <Text fontSize="12px">${formatAmount(baseAsset.liquidity)}</Text>
        </Box>
      </Flex>
      <Box px="25px" mr="auto">
        {totalScore !== 0 && (
          <>
            <Flex fontSize="10px" align="center">
              <Text opacity=".6" mr="10px">
                DAO Score
              </Text>
              <Text color={totalScore > 10 ? "green" : "red"}>
