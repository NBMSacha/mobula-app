...
import {
  defaultTokens, getBlockchainFromId, getIdFromBlockchain, supportedRPCs,
} from "../../../../constants";

export default function Select({
  visible, setVisible, selectToken, oldToken,
}) {
  const web3React = useWeb3React();
  const {
    active, account, chainId, library, connector, activate, deactivate,
  } = useWeb3React();
  ...
  async function updateSearch(search: string, supabase: any, setResults: any) {
    if (ethers.utils.isAddress(search)) {
      ...
      try {
        ...
        setResults([{
          symbol, logo: "/unknown.png", address: search, decimals,
        }]);
      } catch (e) {
      }
    } else {
          ...
          if (token) {
            return (
              <Flex
                ...
                justify="left"
              >
                <Image borderRadius="50%" mr="10px" src={token.logo} h="20px" />
                {" "}
                {token.symbol}
                {" "}
                <Text
                  ...
                  color="grey"
                >
                  {token.address ? `(${token.address.slice(0, 4)}..${token.address.slice(token.address.length - 4, token.address.length)})` : ""}
                </Text>
              </Flex>
            );
          }
