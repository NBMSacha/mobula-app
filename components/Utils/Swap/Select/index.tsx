    ...
    if (ethers.utils.isAddress(search)) {
      ...
      try {
        const contract = new ethers.Contract(
          search,
          ["function symbol() public view returns (string)", "function decimals() public view returns (uint256)"],
          provider
        );
          ...
          const address =
            name.contracts[
              name.blockchains.indexOf(
                name.blockchains.filter((blockchain: string) => getIdFromBlockchain[blockchain] == chainId)?.[0]
              )
            ];
      ...
      <Flex flexWrap="wrap" p="20px" ml="auto" mr="auto">
        {defaultTokens[chainId]?.map((token: any) => (
          <Flex
            ...
            cursor={oldToken?.address == token.address ? "not-allowed" : "pointer"}
            _hover={{ opacity: "0.8" }}
            p="5px 10px"
            mr="10px"
            mb="10px"
          ...
          >
            {token.symbol} <Image ml="5px" src={token.logo} h="20px" />
          </Flex>
        ))}
      </Flex>
            ...
            return (
              <Flex
                onClick={async () => {
                  if (oldToken?.address != token.address) {
                    if (token.address) {
                      const RPC = supportedRPCs.filter(
                        (entry) => entry.name == getBlockchainFromId[web3React.chainId]
                      )[0];
                      const provider = new ethers.providers.JsonRpcProvider(RPC);
                      const contract = new ethers.Contract(
                        token.address,
                ...
                justify="left"
              >
                <Image borderRadius="50%" mr="10px" src={token.logo} h="20px" /> {token.symbol}{" "}
                <Text ml="10px" color="grey">
                  {token.address
                    ? `(${token.address.slice(0, 4)}..${token.address.slice(
