    ...
    >
        ...
        {token.excludedFromCirculation && token.excludedFromCirculation.length > 0
          ? (
            <Text w="30%" px="10px" whiteSpace="nowrap" overflow="hidden" textOverflow="ellipsis">
              Excluded from
              circulation (
              {token.excludedFromCirculation.length}
              )
            </Text>
          ) : <></>}
      </Flex>
      <Flex p={["20px", "20px", "20px 0px", "20px"]} justify="space-around" align="center" w="100%">
        ...
        {token.totalSupply && token.totalSupply.length > 0
          ? (
            <Flex w="30%" direction="column">
              {token.totalSupply.map((contract: string, index: number) => <Box mb="20px" mr="10px"><Contract contract={contract} blockchain={token.chains[0]} /></Box>)}
            </Flex>
          ) : <></>}
        {token.excludedFromCirculation && token.excludedFromCirculation.length > 0
          ? (
            <Flex w="30%" direction="column">
              {token.excludedFromCirculation.map((contract: string, index: number) => <Box mb="20px" mr="10px"><Contract contract={contract} blockchain={token.chains[0]} /></Box>)}
            </Flex>
          ) : <></>}
      </Flex>
    </Flex>
