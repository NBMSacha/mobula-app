...
function Governance() {
    ...
    const [depositAmount, setDepositwAmount] = useState("");
    ...
    const MOBULA_ABI = [
        ...
        "function getLiveProposals(uint256 top) external view returns (Proposal[] memory)", 
    ...
    async function deposit() {
        try{
            ...
            const allowance = await contract.allowance(MOBL_ADDRESS, address)
            let formattedAllowance = await allowance.toString();
    ...
    async function createProposals() {
        ...
        if(provider !== undefined) {
            ...
            const contract = new ethers.Contract(GOVERNOR_ADDRESS, MOBULA_ABI, signer);   
            try {
                const tx = await contract.createProposal(withdrawAmount)
                const txResponse = tx.wait()
                console.log(txResponse)
              } catch (error) {
                console.log(error);
                ...
                <Flex direction="column" bg="rgba(163, 212, 244, 0.25)" ml="auto" mr="auto" borderRadius="25px"  mx={10} className={styles["current-vote"]}>
                    ...
                    <FormControl>
                        ...
                        <Input h="32px" w="90%" bg="rgba(163, 212, 244, 0.2)" border="none" id='deposit' fontFamily="Poppins" borderRadius='10px' type='number' placeholder="3000"
                            ref={depositRef}
                            value={depositAmount}
                            onChange={(e) => setDepositwAmount(e.target.value)}
                        ></Input>
                    </FormControl>
                    <Button bg="linear-gradient(90deg, #003FE1 8.9%, #64D0FF 87.31%)" border="none" w="40%" py="10px" borderRadius="10px" mt="20px" color="white" fontFamily='Poppins' onClick={() => {console.log(withdrawAmount);deposit()}}>Deposit</Button>
                    <FormControl>
