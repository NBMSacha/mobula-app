...
const ChartCryptos = ({ id }) => {
  ...
  const fetchData = async () => {
    ...
    supabase.from('assets').select('*').match({ id: parseInt(id) }).then(r => {
  ...
  useEffect(() => {
    ...
    let gradient = ctx.createLinearGradient(0, 0, 0, (isMobile ? 100 : isGiant ? 600 : 400));
  ...
  function daoBtn() {
    ...
    if (x.matches) {
      if (daoBtn.style.display == "none") {
        ...
        window.document.getElementById('dropdown').style.margin = "0px 0px 150px 0px"
      } else {
        daoBtn.style.display = "none"
        window.document.getElementById('dropdown').style.margin = "0px 0px 0px 0px"
      }
    } else {
      ...
      <div className="App" >
        <div className={styles["chart-main-container"]}>
            ...
            <div className={styles["flex"]}>
              <div className={styles["chart-left-top"]}>
                ...
                <div className={styles["chart-name-box"]}>
                  <div className={styles["chart-token-name"]}><span>{token.name}</span></div>
...

                  {
                    token.price_change_24h < 0 ?
                      <div className={styles["chart-lose"]}>
                        <span><AiOutlineArrowDown className={styles["downArrow"]} /></span>
                        <span>{getTokenPercentage(token.price_change_24h)}%</span>
                      </div> : <div className={styles["chart-gain"]}>
                        <span><AiOutlineArrowUp className="upArrow" /></span>
                        <span>{getTokenPercentage(token.price_change_24h)}%</span>
                      </div>
                  }
                </div>
          ...
          <div className={styles["mobile-showInfo-container"]}>
            <div style={{ "display": "none" }} className={styles["mobile-info-element"]} id="hide">
              <div className={styles["mobile-info-left-column"]}>
                <div className={styles["mobbox"]}>
                  <span className={styles["grey"]}>MARKET CAP</span>
                  <p className={styles["numbers"]}>${formatAmount(token.market_cap)}</p>
                </div>
                <div className={styles["mobbox"]}>
                  <span className={styles["grey"]}>VOLUME (24H)</span>
                  <p className={styles["numbers"]}>${formatAmount(token.volume)}</p>
                </div>
                <div className={styles["mobbox"]}>
                  <span className={styles["grey"]}>FULLY DILUTED MARKET CAP</span>
                  <p className={styles["numbers"]}>${token.market_cap_diluted ? formatAmount(token.market_cap_diluted) : '???'}</p>
                </div>
              </div>
              <div className={styles["mobile-info-right-column"]}>
                <div className={styles["mobbox"]}>
                  <span className={styles["grey"]}>CIRCULATING SUPPLY</span>
                  <p className={styles["numbers"]}>{token.circulating_supply ? formatAmount(token.circulating_supply) : '???'} {token.symbol}</p>
                </div>
                <div className={styles["mobbox"]}>
                  <span className={styles["grey"]}>TOTAL SUPPLY </span>
                  <p className={styles["numbers"]}>{token.total_supply ? formatAmount(token.total_supply) : '???'} {token.symbol}</p>
                </div>
                <div className={styles["mobbox"]}>
          ...
          <div className={styles["chart-bottom-container"]}>
            <div className={styles["chart-bottom-left"]}>
              <div className={styles["left-top-box"]}>
                <span>
                  <p className={`${styles["text-top-chart"]} ${styles["topOne"]}`}>MARKET CAP</p>
                  <p className={styles["text-bottom-chart"]}>${formatAmount(token.market_cap)}</p>
                </span>
                <span>
                  <p className={styles["text-top-chart"]}>VOLUME (24H)</p>
                  <p className={styles["text-bottom-chart"]}>${formatAmount(token.volume)}</p>
                </span>
                <span>
                  <p className={styles["text-top-chart"]}>FULLY DILUTED MARKET CAP</p>
                  <p className={styles["text-bottom-chart"]}>${token.market_cap_diluted ? formatAmount(token.market_cap_diluted) : '???'}</p>
                </span>
                <span>
                  <p className={styles["text-top-chart"]}>CIRCULATING SUPPLY</p>
                  <p className={styles["text-bottom-chart"]}>{token.circulating_supply ? formatAmount(token.circulating_supply) : '???'} {token.symbol}</p>
                </span>
                <span>
                  <p className={styles["text-top-chart"]}>TOTAL SUPPLY </p>
                  <p className={styles["text-bottom-chart"]}>{token.total_supply ? formatAmount(token.total_supply) : '???'} {token.symbol}</p>
                </span>
                <span>
              ...
              <div className={(token.utility_score + token.social_score + token.market_score + token.trust_score == 0 ? styles["left-bottom-box-dis"] : styles["left-bottom-box"])} id="dropdown">
                <button className={styles["notes-boxs"]} id="btnNotes" onClick={() => daoBtn()}>
                ...
                <button className={`${styles["notes-boxs"]} ${styles["disapear"]}`}>
                  <span>Utility</span>
                  <span>{token.utility_score}/5</span>
                </button>
                <button className={`${styles["notes-boxs"]} ${styles["disapear"]}`}>
